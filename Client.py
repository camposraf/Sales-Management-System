...
def create_property_layout():
    return [
        ...
        sg.Column([
            ...
            [sg.Text("Client Name:", font=("Helvetica", 20), size=(14,1)), sg.Input(key="client_name", font=("Helvetica", 20), size=(20,1))],
            [sg.Text("Client Contact:", font=("Helvetica", 20), size=(14,1)), sg.Input(key="client_contact", font=("Helvetica", 20), size=(20,1))],
            [sg.Text("Client Email:", font=("Helvetica", 20), size=(14,1)), sg.Input(key="client_email", font=("Helvetica", 20), size=(20,1))],
            [sg.Text("Client Address:", font=("Helvetica", 20), size=(14,1)), sg.Input(key="client_address", font=("Helvetica", 20), size=(20,1))],
            [sg.Text("", size=(1,1))],
    ...
    while True:
        ...
        if event == "Lookup":
            try:
                conn = sqlite3.connect("primarius.db")
                c = conn.cursor()
                c.execute("SELECT id, name, location, price, status, client_name, client_contact, client_email, client_address, photo, photo2 FROM properties WHERE id=?",
                          (values["lookup_id"],))
                row = c.fetchone()
                conn.close()
                if row:
                    ...
                    window["client_name"].update(row[5])
                    window["client_contact"].update(row[6])
                    window["client_email"].update(row[7])
                    window["client_address"].update(row[8])
                    if row[9]:
            ...
            c.execute("""INSERT INTO properties (id, name, location, price, status, client_name, client_contact, client_email, client_address, photo, photo2) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                      (new_id, values["name"], values["location"], values["price"], values["status"],
                       values["client_name"], values["client_contact"], values["client_email"], values["client_address"],
            ...
            conn = sqlite3.connect("primarius.db")
            c = conn.cursor()
            c.execute("SELECT id, name, location, price, status, client_name, client_contact, client_email, client_address FROM properties")
            rows = c.fetchall()
            conn.close()
...
def payments_window():
    ...
    while True:
        ...
        if event in (sg.WIN_CLOSED, "Back"):
            ...
            c = conn.cursor()
            c.execute("INSERT INTO payments (id, property_id, amount, date, status) VALUES (?, ?, ?, ?, ?)",
                      (next_id("payments", "PAY"), values["property_id"], values["amount"], values["date"], values["status"]))
            conn.commit()
            conn.close()
